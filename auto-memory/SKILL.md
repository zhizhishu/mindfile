...
- **② 耐久类型标签**(record `--durability axiom|pattern|workaround|project-state` → frontmatter `metadata.durability`): 让保留由**内在耐久度**驱动而非 mtime——`axiom`(铁律)永不归档、`workaround`(工具/版本相关)会过气加冷权重。治"真铁律放 40 天没碰被误判冷"。(2026-07-18 起**四类均驱动冷打分**: `axiom` 锁热永不归档 / `workaround`·`project-state` 加冷权重更易归档 / `pattern` 加热权重更抗归档。)
- **⑤ 时态失效标记**(会漂的事实): 会随时间腐烂的 fact(健康状态/在线数/余额/"当前"状态/快变的外部约束)在正文标 `valid-as-of: YYYY-MM`; 复核时**超期的先重新核实、不盲信**——标 `needs-review`(不自动删、不静默淘汰)。这是"浮动数字别进长期层"的另一面: 结构进长期层、会漂的数带日期快照或指回带日期的历史条。**2026-07-18 起 `automemory_audit.py` 已半自动化**: 扫正文 `valid-as-of` 超 6 个月(阈值可调)的自动标 `stale-valid-as-of`(WARN), 已含 `superseded`/`needs-review` 标记的汇总进 `temporal_review`(只报不删, 复核仍由 agent/人做)。
